...
- Non-convex adjustment costs (quadratic plus fixed costs)
- TFP/Profit shocks
- No taxes
- No external finance
- Partial equilibrium
...
@numba.jit
def VFI_loop(EV, C, betafirm, Pi, sizez, sizek, Vmat):
    '''
    ------------------------------------------------------------------------
    ...
    EV       = (sizez, sizek) matrix, expected value function (expectations
               over z')
    C        = (sizek, sizek) matrix, cash flow values for each possible
    ...
    for i in range(sizez):  # loop over z
        for j in range(sizek):  # loop over k
            for m in range(sizek):  # loop over k'
                Vmat[i, j, m] = C[i, j, m] + betafirm * EV[i, m]
...
psi1      = scalar >= 0, fixed cost of adjusting the capital stock (as a
...
sigma_z   = scalar > 0, standard deviation of profit/productivity shocks to
...
C  = (sizez, sizek, sizek) array, cash flow values for each possible
...
EVnc = np.empty((sizez, sizek))  # initialize EVnc
...
while VFdist > VFtol and VFiter < VFmaxiter:
    ...
    EVnc[:, dens:] = EV[:, :sizek - dens]
    EVnc[:, :dens] = np.tile(np.reshape(EV[:, 0], (sizez, 1)), (1, dens))
    Vmat = VFI_loop(EV, e, betafirm, Pi, sizez, sizek, Vmat)
    Vnc = e_nc + betafirm * EVnc
    Vc = Vmat.max(axis=2)  # apply max operator to Vmat (to get V(k))
...
output_path = os.path.join(output_dir, 'V_firm5')
...
plt.title('Value Function - stochastic firm w/ adjustment costs')
output_path = os.path.join(output_dir, 'V_firm5_iter')
plt.savefig(output_path, dpi=200, bbox_inches="tight")
# plt.show()
plt.close()
...
plt.title('Policy Function, Next Period Capital - stochastic firm w/ ' +
          'adjustment costs')
output_path = os.path.join(output_dir, 'Kprime_firm5')
plt.savefig(output_path, dpi=200, bbox_inches="tight")
# plt.show()
plt.close()
...
plt.title('Policy Function, Investment - stochastic firm w/ adjustment ' +
          'costs')
output_path = os.path.join(output_dir, 'invest_firm5')
plt.savefig(output_path, dpi=200, bbox_inches="tight")
# plt.show()
plt.close()
...
plt.title('Policy Function, Investment - stochastic firm w/ adjustment ' +
          'costs')
output_path = os.path.join(output_dir, 'invest_z_firm5')
plt.savefig(output_path, dpi=200, bbox_inches="tight")
# plt.show()
plt.close()
